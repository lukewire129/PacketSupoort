...
namespace BytePacketSupport.Integrity.CRC
{
    ...
    public class CRC8 : ErrorDetection
    {
        public CRC8Type Type { get; } = CRC8Type.Classic;

        public CRC8(CRC8Type type = CRC8Type.Classic)
        {
            Type = type;
        }
        ...
        {
            byte ret = 0x00;
            if (Type == CRC8Type.Classic)
                ret = ComputeClassicCRC8 (source);
            else if (Type == CRC8Type.Maxim)
                ret = (ComputeMaximCRC8 (source));
        ...
        private static readonly byte[] _crc8ClassicTable = // CCITT?
{
        ...
        private byte ComputeClassicCRC8(ReadOnlySpan<byte> data)
        {
            if (data.Length == 0)
                return 0;
            ...
            foreach (var dt in data)
                crc = _crc8ClassicTable[(crc ^ dt) & 0xff];

            return crc;
        }

        private static byte ComputeMaximCRC8(ReadOnlySpan<byte> data)
        {
            if (data.Length == 0)
                return 0;
